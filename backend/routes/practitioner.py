...
def get_new_patients(
    ...
    for case in recent_cases:
        ...
        if patient is None:
            continue
        status_payload = _compute_case_status(case)
        response.append(
...
def update_patient_case(
    ...
    if not updates:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Aucune donn\xe9e \xe0 mettre \xe0 jour.",
        )
    ...
    if "procedure_type" in updates:
        ...
        if raw_type is not None:
            try:
                ...
                raise HTTPException(status_code=400, detail="Type de proc\xe9dure invalide.") from exc
    ...
    db.commit()
    db.refresh(case)
    return _compute_case_status(case)
...
def reschedule_appointment(
    ...
    patient = appointment.user
    if case is None or patient is None:
        raise HTTPException(status_code=400, detail="Le rendez-vous n'est pas reli\xe9 \xe0 un dossier patient.")

    status_payload = _compute_case_status(case)
    return _appointment_entry(appointment, patient, case, status_payload)
...
def create_practitioner_appointment(
    ...
    try:
        ...
        message = str(exc)
        status_code = status.HTTP_409_CONFLICT if "deja" in message.lower() or "déjà" in message.lower() else status.HTTP_400_BAD_REQUEST
